    ...
    const fetchAllUsers = async () => {
        ...
        try {
            ...
            const [pendingResponse, approvedResponse] = await Promise.all([
                axios.get("/api/v1/users/pending"),
                axios.get("/api/v1/users/approved")
            ]);

            if (pendingResponse.data.success && approvedResponse.data.success) {
                // Combine the users and sort by status (pending first) and then by date
                const pendingUsers = pendingResponse.data.data.users.map(user => ({
                    ...user,
                    status: "pending"
                }));

                const approvedUsers = approvedResponse.data.data.users.map(user => ({
                    ...user,
                    status: "approved"
                }));

                // Sort: pending first, then by date (newest first)
                const combinedUsers = [...pendingUsers, ...approvedUsers].sort((a, b) => {
                    // First sort by status (pending first)
                    if (a.status === "pending" && b.status === "approved") return -1;
                    if (a.status === "approved" && b.status === "pending") return 1;

                    // Then sort by date (newest first)
                    const dateA = a.status === "pending" ? new Date(a.createdAt) : new Date(a.approvalDate);
                    const dateB = b.status === "pending" ? new Date(b.createdAt) : new Date(b.approvalDate);
                    return dateB - dateA;
                });
...
export default withAuth(PendingUsers); 